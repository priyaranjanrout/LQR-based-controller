...
import rclpy
from rclpy.node import Node
>>>>>>> 72e42a784558618bfc23e2f48189d14c6790de3c
...
<<<<<<< HEAD
		...
		#self.thrust_command = self.create_publisher(RollPitchYawrateThrust, '/rotors/command/roll_pitch_yawrate_thrust', 10)
...
<<<<<<< HEAD
	...
	def map_u_to_pwm(self):
		pwm_base = 660# This is the base PWM value for the motors

		self.pwm[0] = np.clip(pwm_base + (self.u[0]), 600, 800)  # Ensure the PWM values are within a valid range
		self.pwm[1] = np.clip(pwm_base + (self.u[1]), 600, 800)
		self.pwm[2] = np.clip(pwm_base + (self.u[2]), 600, 800)
		self.pwm[3] = np.clip(pwm_base + (self.u[3]), 600, 800)
